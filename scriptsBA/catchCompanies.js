...
elasticClient.ping({
    ...
    if (error) {
        ...
        fs.readFile('tokens.json', 'utf8', function read(err, data) {
            ...
            fs.readFile('lastIndex.json', 'utf8', function read(err, data) {
                if (err) throw err;
                g_startIndex = JSON.parse(data).lastIndex
    ...
    var interval = setInterval(function(){
        getCompanyForIndex(startIndex)
            ...
            .catch(function(errorObj){
                ...
                if(errorCount > 20 || errorObj.error == 'Throttle limit for calls to this resource is reached.'){
                    deferred.reject()
...
function saveToDb(company){
    ...
    elasticClient.index({
        ...
        if(err) {
            errorIds.ids.push(company.id)
            if(errorIds.ids.length > 1000) {
                saveToFile(errorIds)
                errorIds.ids = []
            }
            deferred.reject({id: company.id, error: err})
        }else
